...
GlasserTable.index = range(1,181)
...
#Change the actual index to 1-->180
GlasserTable_sorted_PrimarySection.index = range(1,181)
#Insert a column that describes the sorted dataframe new index from 1-->180
GlasserTable_sorted_PrimarySection.insert(loc=1, column='Sorted_PrimarySection_Index', value=range(1,181))
...
Targets = pd.DataFrame({'Id': GlasserTable_sorted_PrimarySection_Index_ThreeDigits, 'Label': GlasserTable_sorted_PrimarySection.AreaName, 'color': GlasserTable_sorted_PrimarySection.ColourRGB, 'Modularity Class': GlasserTable_sorted_PrimarySection.SectionsBOLD}, index=range(1,181))
print('Targetsdf : \n', Targets)
...
for seed in seeds:
    for i,this_k in enumerate(cluster_range): #For 0,2 1,3 2,4
        ...
        if this_k==2:
            this_k_ColourRGB = ['255,0,0','0,255,0'] #Cluster01=Red, #Cluster02=Blue
        elif this_k==3:
        ...
        individualclusterstoadd = pd.DataFrame({'Id': IdlistClusters, 'Label': labellist, 'color': this_k_ColourRGB, 'Modularity Class': ModularityClassClusters}, index=range(181,181+this_k))
        ...
        EdgesSourceId = np.zeros((180,this_k), dtype=int)
        EdgesSourceNames = list()
        for i,this_label in enumerate(range(1,this_k+1)):
            EdgesSourceId[:,i] = this_label+180
        ...
        #Use functools reduce to remove iterative lists into a single list i.e. [180xCluster01], [180xCluster02], [180xCluster03] to [180xCluster01, 180xCluster02, 180xCluster03]
        EdgesSourceNames = reduce(lambda x,y: x+y, EdgesSourceNames)
        #Reshape the edges source col current with shape (180,numClusters) to 180*numClusters with order 'F' (i.e. 180x'181' --> 180x'182' --> 180x'130')
        EdgesSourceId = np.reshape(EdgesSourceId, (180*this_label,), order='F')
        ...
        for i,thisk_thisCluster_npz in enumerate(individualClusters_thisk_npzList):
            ...
            df_thisCluster = pd.DataFrame({'targetnames': GlasserTable.AreaName, 'ConnectivityScore': targetAvg_across_voxels_thisCluster, 'ConnectivityScoreNormalized': quotients}, index=range(1,181))
            ...
            df_thisCluster_sorted_PrimarySection.index = range(1,181)
            #Insert a column that describes the sorted dataframe new index from 1-->180
            df_thisCluster_sorted_PrimarySection.insert(loc=1, column='Sorted_PrimarySection_Index', value=range(1,181))
            ...
            df_sorted_thisCluster_InsertRank['SortedRank'] = pd.Series(reversed(range(1,181)),index=(df_sorted_thisCluster.index))

            df_thisCluster_sorted_PrimarySection_InsertRank = df_thisCluster_sorted_PrimarySection
            df_thisCluster_sorted_PrimarySection_InsertRank['SortedRank'] = pd.Series(reversed(range(1,181)),index=(df_sorted_thisCluster.index))
        ...
        Edgesdf = pd.DataFrame({'Source': EdgesSourceId, 'SLabel': EdgesSourceNames, 'Target': TargetsMultiplied.Id, 'TLabel': TargetsMultiplied.Label, 'ConnectivityScore': df_ALLCLUSTERS_InsertRank.ConnectivityScore, 'ConnectivityScoreNormalized': df_ALLCLUSTERS_InsertRank.ConnectivityScoreNormalized, 'Rank_in_Cluster': df_ALLCLUSTERS_InsertRank.SortedRank, 'Weight': df_ALLCLUSTERS_InsertRank.Weight}, index=range(0,this_k*180))
        print('\n Shape of this Edges is %s and Edgesdf = \n %s  ' % (Edgesdf.shape, Edgesdf))


        print('\n ------ Saving Nodesdf and Edgees df to csv ------ \n ')

        if not os.path.exists('results/gephi_input_NodesandEdges_tables'):
            os.mkdir('results/gephi_input_NodesandEdges_tables')

        #Create savepaths
        savepathNodesdf = 'results/gephi_input_NodesandEdges_tables/GephiNodes_' + seed + '_k-' + str(this_k) + '_Sorted_Overarching22Categories_WithRGBColours.csv'
        savepathEdgesdf = 'results/gephi_input_NodesandEdges_tables/GephiEdges_' + seed + '_k-' + str(this_k) + '_Sorted_Overarching22Categories_NormalizedWeight.csv'
